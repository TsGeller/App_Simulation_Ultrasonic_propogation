...
[m n]=find(target_plane_CT>CT_th2);
[k1,av] = convhull(n,m);
x_convhull=n(k1); 
y_convhull=m(k1);
% figure()
...
x_circle1_cut=[];
y_circle1_cut=[];
i = find (y_circle1<ACC_target_xy(2)-(focol_distance/2));
...
angel=foundTransductor(x_circle1_cut,y_circle1_cut,angle_inside_all,target_plane_CT,ACC_target_xy,x_convhull,y_convhull,x0,y0);
x_angel=[];
y_angel=[];
angle_angel=[];
j=1;
...
% title(['MRI image include target，','rotate angle is ',num2str(rotate_angle),'°'])
 ...
 %% Déterminer les coordonnées du point cible et les coordonnées du point incident ultrasonore à partir de l'image planaire contenant le point cible, voici un exemple
            ...
            diameter1=0;
            if(mod(round(source_diameter/kwave_step*1000),2)==0)
                diameter1 = round(source_diameter/kwave_step*1000)+1;
            else
                diameter1 = round(source_diameter/kwave_step*1000);
            end
            bowlVolshow1 = makeBowl(size(MRI_rotate),transducer_position_new1,round(source_roc/kwave_step*1000),diameter1,[m ,n, k]);
...
CT_final(CT_final<CT_th2&CT_final>CT_th1)=CT_th2;
...
sensor.record = {'p_max'};
%Le paramètre à enregistrer est la valeur maximale de la pression acoustique
...
sensor.record_start_index =1 ;%%kgrid.Nt - record_periods * PPP + 1
