classdef Simulation < handle
    properties
        %Pour label
        %Variable ouverture du fichier CT
        animalName = 'Macaque'
        filenameCT
        filepath1CT
        info1
        ...
        CT
        %%%
        %Variable ouverture du fichier IMR
        filenameIMR
        filepathIMR
        info2
        V2
    ...
    methods
        ...
        function obj = openCTFile(obj)
            [obj.filenameCT, obj.filepath1CT] = uigetfile('*.dcm');
            % Sélectionnez simplement n'importe quel fichier DICOM pour obtenir les informations de l'image C
            filepath2 = [obj.filepath1CT obj.filenameCT];
            obj.info1 = dicominfo(filepath2); % Lire les informations des données DICOM
            [obj.V1, obj.spatial1, obj.dim1] = dicomreadVolume(obj.filepath1CT);
            ...
            obj.V1 = squeeze(obj.V1); % Éliminer les côtés de longueur 1
            obj.D1 = single(obj.V1); % Convertir en simple précision
            clear V1;
            %% Échelle de gris à la valeur CT
            obj.CT = obj.D1 .* obj.info1.RescaleSlope + obj.info1.RescaleIntercept;
            return;
        end
        function obj = openIRMFile(obj)
            %% Ouvrir l'image IRM
            % filename2 est le nom du fichier DICOM que vous souhaitez utiliser
            [obj.filenameIMR, obj.filepathIMR] = uigetfile('*.dcm');
            filepath2 = [obj.filepathIMR obj.filenameIMR];
            obj.info2 = dicominfo(filepath2);
            [obj.V2, obj.spatial2, obj.dim2] = dicomreadVolume(obj.filepathIMR);
            % V: Données brutes (4D), spatial: Résolution spatiale et coordonnées, dim: dimension:dimension
            obj.V2 = squeeze(obj.V2); % Éliminer les côtés de longueur 1
            obj.D2 = single(obj.V2); % Convertir en simple précision
            clear V2;
            obj.MRI = obj.D2;
            return ;
        end
        function obj = makeSimulation(obj)
            obj.simulationWorker();

        end
        function obj = simulationWorker(obj)
            % Fonction de travail pour effectuer la simulation, first part
            %% Acquérir la résolution des images CT et IRM

            disp('Acquérir la résolution des images CT et IRM')
            MRI_space=[];%Résolution tridimensionnelle de l'IRM (mm)
            MRI_space(1:2)=obj.info2.PixelSpacing;
            MRI_space(3)=obj.info2.SliceThickness;
            ...
            disp('Définissez les limites supérieures et inférieures et les couches de données CT du crâne')
            obj.CT(obj.CT<obj.CT_th1)=obj.CT_th1;
            obj.CT(obj.CT<obj.CT_th2&obj.CT>obj.CT_th1)=obj.CT_th2;
            obj.CT(obj.CT>obj.CT_th3)=obj.CT_th3;
            ...
            disp('Sélectionnez le même emplacement')
            axial_plane_center1 = round(size(obj.CT,3)/2);
            axial_plane_center2 = round(size(obj.MRI,3)/2);
            CT_im=obj.CT(:,:,axial_plane_center1);
            MRI_im=obj.MRI(:,:,axial_plane_center2);
            %%CTandMRI=CT_im*0.3+MRI_im; % Image de superposition

            %             % Créez une figure pour afficher les images CT et MRI
            %             fig = figure();
            %             subplot(131)
            %             imagesc(CT_im)
            %             axis equal
            ...
            disp('Calculer le pas spatial de l onde k')
            wave_length=obj.wave_speed/(obj.wave_frequency*1e6)*1000;%longueur d'onde mm
            kwave_step=wave_length/obj.ppw;% La taille de pas de chaque point de la grille dans k-wave


            %% Interpolation des données brutes CT et IRM
            disp('Interpolation des données brutes CT et IRM')
            [m,n,k]=size(obj.CT);
            ...
            CT_inter = interp3(x,y,z,obj.CT,xq,yq,zq);%Données CT interpolées
            MRI_inter = interp3(x,y,z,obj.MRI,xq,yq,zq);%Données IRM interpolées
            ...
            disp('Définissez les limites supérieure et inférieure')
            CT_inter(CT_inter<obj.CT_th1)=obj.CT_th1;
            CT_inter(CT_inter<obj.CT_th2&CT_inter>obj.CT_th1)=obj.CT_th2;
            CT_inter(CT_inter>obj.CT_th3)=obj.CT_th3;

            %% Recadrez ou amplifiez les données brutes pour réserver une place au transducteur
            disp('Recadrez ou amplifiez les données brutes')
            ...
            disp('Trouvez le plan où se trouve le point cible')
            
            coordinates= vv3Simplified(MRI_inter);
            if(coordinates == 0 )
             error("None target");
            end
            zM = coordinates(3);
            xM = coordinates(1);
            yM = coordinates(2);
            target_plane_position=zM;%Trouver le plan où se trouve l'ACC macaque k: 99, 140, 110, macaque E: 98,120, 120

            %% Dessinez le plan où se trouve la cible
            disp('Dessinez le plan où se trouve la cible')
            ...
            disp('Déterminer les coordonnées restantes de la cible')
            ACC_target_xy=[xM,yM];%cible
            %             figure
            %             imagesc(target_plane)
            %             axis equal
            %             axis tight
            ...
            [m n]=find(target_plane_CT>obj.CT_th2);
            [k1,av] = convhull(n,m);
            x_convhull=n(k1);
            y_convhull=m(k1);
            %             figure()
           ...
           %La distance focale réelle du transducteur focalisé (calculée à partir du sommet du transducteur)
            ...
            x0 = (ACC_target_xy(1));
            y0 = (ACC_target_xy(2));
            r = obj.focol_distance/kwave_step;%combien de grilles le rayon occupe
            ...
            i = find (y_circle1<ACC_target_xy(2)-(obj.focol_distance/2));
            %%erreur x4
            ...
            for i1=1:length(x_circle1_cut)
                ...
                for i3=1:1:length(x_convhull)-1
                    u = ((x1-x_convhull(i3))*(y1-y2) - (y1-y_convhull(i3))*(x1-x2)) / ((x1-x2)*(y_convhull(i3)-y_convhull(i3+1))-(y1-y2)*(x_convhull(i3)-x_convhull(i3+1)));
                    t = ((x1-x_convhull(i3))*(y_convhull(i3)-y_convhull(i3+1)) - (y1-y_convhull(i3))*(x_convhull(i3)-x_convhull(i3+1))) / ((x1-x2)*(y_convhull(i3)-y_convhull(i3+1))-(y1-y2)*(x_convhull(i3)-x_convhull(i3+1)));
                ...
                angle_inside_all(i1)=angle_inside;
                    ...
                    error('one angle was not found')
                end
               
            end

            %% Dessinez un histogramme de la distribution des angles
            disp(' Dessinez un histogramme de la distribution des angles')
            %             figure()
            ...
            angle_angel=[];
            j=1;
            diff_angel=abs(angle_inside_all-obj.angel);
            ...
            % Créez un slider pour ajuster l'angle
            hSlider = uicontrol('Style', 'slider', 'Min', 60, 'Max', 120, 'Value', 90, 'Position', [5, 5, 120, 20], 'Callback', @updatePlot);

            % Créez un bouton "Enregistrer"
            hSaveButton = uicontrol('Style', 'pushbutton', 'String', 'Enregistrer', 'Position', [140, 5, 80, 20], 'Callback', @saveAngle);

            % Créez un axe pour afficher l'image et les graphiques
            ax = axes('Parent', hFig);

            % Initialisez l'angle avec la valeur de départ (0 degré)
            angle_degrees = 90;
            % Appelez la fonction pour tracer le graphique initial
            updatePlot(hSlider);

            % Fonction de mise à jour de l'image en fonction de la valeur du slider
            function updatePlot(src, ~)
                % Obtenez la valeur actuelle du slider
                angle_degrees = src.Value;

                % Effacez le contenu précédent de l'axe
                cla(ax);
                x_angel=[];
                y_angel=[];
                angle_angel=[];
                j=1;
                diff_angel=abs(angle_inside_all-angle_degrees);
                k=find(diff_angel==min(diff_angel));
                x_angel=x_circle1_cut(k);
                y_angel=y_circle1_cut(k);
                angle_angel=angle_inside_all(k);
                % Affichez l'image CT et d'autres éléments
                imagesc(target_plane_CT, 'Parent', ax);
                xlabel(ax, 'z distance');
                ylabel(ax, 'y distance');
                axis(ax, 'equal');
                axis(ax, 'tight');
                colormap(ax, gray);
                title(ax, ['CT image and target, The angle of ultrasound incidence is ', num2str(angle_degrees), '°']);
                hold(ax, 'on');
                plot(ax, ACC_target_xy(1), ACC_target_xy(2), 'r+');
                plot(ax, x_circle1_cut, y_circle1_cut, 'b+');
                plot(ax, x_convhull, y_convhull, 'r');
                plot(ax, [x_angel(1) x0], [y_angel(1), y0], 'b');
                drawnow;
            end

            % Fonction pour enregistrer l'angle
            function saveAngle(~, ~)
                % Code pour enregistrer l'angle où vous le souhaitez
                % Par exemple, vous pouvez utiliser la fonction d'écriture de fichier
                % ou l'enregistrer dans une variable globale
                disp(['Angle enregistré : ', num2str(angle_degrees), '°']);
                close(hFig);
            end

            % Boucle while pour mettre à jour en continu l'image
            while ishandle(hFig)
            ...
            Lax1=find(transform_matrx_rotate==200);%
            s=size(MRI_rotate);%Calculer la taille d'un tableau tridimensionnel
            [m,n,k]=ind2sub(s,Lax1(round(length(Lax1)/2)));%Convertir un indice unique maximum en indice multidimensionnel tridimensionnel
            targrt_position_new1=[m,n,k];
            transducer_position_new1=[m-distance_tran_tar,n,k];  
            diameter1=0;
            if(mod(round(obj.source_diameter/kwave_step*1000),2)==0)
                diameter1 = round(obj.source_diameter/kwave_step*1000)+1;
            else
                diameter1 = round(obj.source_diameter/kwave_step*1000);
            end
            bowlVolshow1 = makeBowl(size(MRI_rotate),transducer_position_new1,round(obj.source_roc/kwave_step*1000),diameter1,[m ,n, k]);
            bowlVolshow1 = bowlVolshow1*900;
            bowlVolshow1 = MRI_rotate+bowlVolshow1;
            

            dimensioncut =cropMtri(bowlVolshow1,targrt_position_new1);
            clear bowlVolshow1 CT_inter MRI_inter MRI_im MRI_add CT_add CT_im xq yq zq 
            ...
            row_set=[dimensioncut(3),dimensioncut(4)];
            column_set=[dimensioncut(1),dimensioncut(2)];
            page_set=[dimensioncut(5),dimensioncut(6)];
            ...
            disp('observer les position relative')
            ...
            disp('<calculer l epaisseur du crane')
            Transcranial_HU=CT_final_Sagittla(v22(2):v11(2),v11(1));
            position_skll=(0:abs(v11(2)-v22(2))).*kwave_step;

            skull_index=find(Transcranial_HU>obj.CT_th2);%Trouver le point de coupure du crâne
            ...
            CT_final(CT_final<obj.CT_th1)=obj.CT_th1;
            CT_final(CT_final<obj.CT_th2&CT_final>obj.CT_th1)=obj.CT_th2;
            CT_final(CT_final>obj.CT_th3)=obj.CT_th3;

            %% définir les constantes acoustiques
            disp('definir constante acoustique')
            HU_max=max(CT_final(:));
            HU_water=obj.CT_th1;
            swater=obj.wave_speed;%Règle la vitesse du son dans l'eau, correspondant au minimum HU
            dbone=1900;%Régler sur la densité dans l'os cortical, correspondant au plus grand HU
            f=obj.wave_frequency;%Tester la fréquence ultrasonique pour ce coefficient d'atténuation (MHz)
            ...
            density=(CT_final-HU_water).*((dbone-obj.dwater)/(HU_max-HU_water))+obj.dwater;%Calculer la densité
            speed=(CT_final-HU_water).*((obj.sbone-swater)/(HU_max-HU_water))+swater;%Calculer la vitesse
            k=(dbone-density)./(dbone-obj.dwater);%Calculer la porosité
            atta=amin+(amax-amin).*(k.^0.5);%Calculer le facteur d'atténuation
            density(CT_final==obj.CT_th2)=obj.dtissue;%densité des tissus mous
            speed(CT_final==obj.CT_th2)=obj.stissue;%vitesse des tissus mous
            atta(CT_final==obj.CT_th2)=atissue;%coefficient d'atténuation des tissus mous
            atta(CT_final==obj.CT_th1)=amin;%Régler l'atténuation de l'eau sur amin
            ...
            disp('definir les conditions de simulationde l onde')
            ...
            medium.density = obj.dwater * ones(Nx, Ny, Nz);       % [kg/m^3]
            ...
            source_f0       = obj.wave_frequency*1e6;      % source frequency [Hz]
            %Définir la grille horaire
            ('definir la grille horaire');
            % cfl             = 0.5;      % CFL Numéro
            %%cfl             = 0.2;      % CFL Numéro
            %%obj.PPP = round(obj.ppw / cfl);%Combien de cycles par point
            dt = 1 / (obj.PPP * source_f0);
            ...
            disp('définir un vide kwave')
            karray = kWaveArray('BLITolerance', 0.01, 'UpsamplingRate', 10);
            % Augmenter le transducteur en forme de bol
            karray.addBowlElement(bowl_pos, obj.source_roc, obj.source_diameter, focus_pos);

            %% définition binary mask
            source.p_mask = karray.getArrayBinaryMask(kgrid);
            % Définir la série temporelle du signal
            source_sig = createCWSignals(kgrid.t_array, source_f0, obj.source_amp, 0);
            ...
            sensor.record = {'p_max'};%ajouter 'p' pour avoir la variations acoustique dans le temps .%Le paramètre à enregistrer est la valeur maximale de la pression acoustique
            ...
            sensor.record_start_index = 1;%% La cible n'est pas exposé avant 200
            ...
            input_args = {'DisplayMask', display_mask, 'PlotLayout', true, 'PMLInside', false, 'PlotPML', false,...
                ...
                [-1, 1] * obj.source_amp};%cpu opération
...
%             %affiche la variation acoustique d'un voxel à travers le temps
%             Tt = [0:size(sensor_data.p,4)-1]*dt;
%             figure % figure utilisable en ajoutant le parametre p a sensord.record.
%             plot(Tt,squeeze(sensor_data.p(m,n,k,:)))%%affiche la variation acoustique de la cible à travers le temps
%             xlabel("Temps [ms]")
        ...
        function data = exportData(obj)
            attributNotSave = ["MRI","D2",'CT','D1','dim2','spatial2','V2','info2','dim1','spatial1','V1','info1'];
            ...
            for i = 1:length(props)
                ...
                % Vérifie si prop_name est dans le tableau attributNotSave
                if ~ismember(prop_name, attributNotSave)
                    prop_value = obj.(prop_name);
                    data.(prop_name) = prop_value;
                end
            end
        end
        function stringPath = loadConfig(obj)

            [pathfileConfig, path] =     uigetfile('*.json');

            if isequal(pathfileConfig, 0)
                % L'utilisateur a annulé la sélection du fichier
                data = struct(); % Retourne une structure vide
                return;
            end

            fullFilePath = fullfile(path, pathfileConfig);

            try
                jsonString = fileread(fullFilePath); % Lecture du contenu du fichier JSON en tant que chaîne
                disp(jsonString); % Afficher la chaîne JSON
                % Caractère spécial de séparation
                strArray = strsplit(jsonString, '\n');
                length1 =length(strArray)-1;
                data= strArray(length1);
                % Transformation de la chaîne JSON en structure
                value = jsondecode(cell2mat(data));
                obj.loadData(value);
                disp('Configuration chargée avec succès.');
            catch exeption
                error("Error: "+exeption.message);
                stringPath = "Config not find";
                return;
            end
            stringPath = pathfileConfig;

        end
        function loadData(obj, data)
            % Liste des propriétés de la classe Simulation
            properties = {'animalName', 'filenameCT', 'filepath1CT', 'info1', 'V1','spatial1', 'dim1', 'D1', 'CT', 'filenameIMR', 'filepathIMR', ...
                'info2', 'V2', 'spatial2', 'dim2', 'D2', 'MRI', 'wave_speed','CT_th1', 'CT_th2', 'CT_th3', 'wave_frequency', 'ppw', 'angel', ...
                'dwater', 'stissue', 'dtissue', 'sbone', 'source_roc','focol_distance', 'source_diameter','source_amp'};

            for i = 1:length(properties)
                if isfield(data, properties{i})
